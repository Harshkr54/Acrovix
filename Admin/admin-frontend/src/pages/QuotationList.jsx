...
import { FileText, Plus, AlertCircle, ChevronLeft, ChevronRight, File, Trash2, ShoppingCart, Eye, CheckCircle, XCircle, RefreshCw, History, Send, Pencil } from 'lucide-react';
...
export default function QuotationList() {
    ...
    const getStatusStyle = (rawStatus) => {
        ...
        switch(status) {
            ...
            case 'SENT': return 'text-brand-primary';
            case 'ACCEPTED': return 'text-brand-success';
    ...
    const renderSourceBadge = (q) => {
        ...
        if (source === 'ENQUIRY' || (!source && q.enquiry)) {
            return (
                <span className="inline-flex items-center text-[10px] font-bold uppercase tracking-wider text-[var(--color-brand-primary)] bg-[var(--color-brand-primary)]/10 px-2 py-0.5 rounded-md mt-1">
                    WEBSITE ENQUIRY
                </span>
            );
        }
        ...
        return (
            <span className="inline-flex items-center text-[10px] font-bold uppercase tracking-wider text-brand-teal bg-[#14B8A6]/10 px-2 py-0.5 rounded-md mt-1">
                {label}
            </span>
        );
    };
                    ...
                    <table className="acx-table">
                        <thead>
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-[11px] font-bold text-text-muted uppercase tracking-wider bg-bg-acx-card hidden sm:table-cell">Created</th>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-center align-top">
                                        <div className="flex items-center justify-center space-x-2">
                                            <button
                                                ...
                                                className="inline-flex items-center px-3 py-1.5 bg-bg-acx-card hover:bg-bg-hover disabled:opacity-50 border border-border-subtle rounded-xl text-[12px] font-semibold text-text-primary transition-colors shadow-sm"
                                                title="View PDF"
                                            >
